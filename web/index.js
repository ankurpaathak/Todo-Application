...
function addNewRowInShowAllTable(todo){
    ...
    cell4.innerHTML = "<button onclick='deleteRow("+todo.id+",this)' class='btn btn-danger'>Delete</button>";
    cell5.innerHTML = "<button onclick='editRow("+todo.id+",this)' class='btn btn-info'>Edit</button>";
}

function editRow(id, x){
    currentUpdateRow = x.parentElement.parentElement;
    var requestOptions = {
        method: 'GET',
        redirect: 'follow'
    };
...
function updateTodo(){
    ...
    updateTodoOnServer(currentUpdateId, title, desc);
}
function updateTodoOnServer(id, title, desc){
    var myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");
    var raw = JSON.stringify({
        "title": title,
    ...
    fetch("http://localhost:8080/todo/update?id=" + id, requestOptions)
        ...
        .then(result => {
            ...
            currentUpdateRow.cells[1].innerHTML = title
            currentUpdateRow.cells[2].innerHTML = desc
